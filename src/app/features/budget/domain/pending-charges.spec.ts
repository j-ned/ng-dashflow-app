...
const BASE = {
  ...
  recurringTransfers: [] as RecurringEntry[], ignored: new Set<string>(),
  salaryDay: 5, currentDay: 20, currentMonth: '2026-06', txs: [] as AccountTransaction[],
};

describe('buildPendingCharges', () => {
  it('inclut une dépense éligible et calcule direction/date/montant', () => {
    const out = buildPendingCharges({ ...BASE, monthlyExpenses: [re({ id: 'e', dayOfMonth: 5, amount: 80 })] });
    expect(out).toHaveLength(1);
    expect(out[0]).toMatchObject({ direction: 'expense', suggestedDate: '2026-06-05', suggestedAmount: 80 });
  });

  it('mappe direction selon le type', () => {
    const inc = buildPendingCharges({ ...BASE, incomes: [re({ id: 'i', type: 'income' })] })[0];
    const tr = buildPendingCharges({ ...BASE, recurringTransfers: [re({ id: 't', type: 'transfer' })] })[0];
  ...
  it('exclut une entrée déjà postée', () => {
    const tx: AccountTransaction = {
      id: 't', accountId: 'a', amount: 100, direction: 'expense', toAccountId: null,
      date: '2026-06-05', category: null, note: null, memberId: null, recurringEntryId: 'e',
    };
    expect(buildPendingCharges({ ...BASE, monthlyExpenses: [re({ id: 'e' })], txs: [tx] })).toHaveLength(0);
  });
});
