...
const BASE = {
  ...
  outgoingTransfers: [] as RecurringEntry[],
  incomingTransfers: [] as RecurringEntry[],
  salaryDay: 5,
  currentDay: 20,
...
describe('buildTimelineEvents', () => {
  it('revenu → sign +, dépense → sign -', () => {
    const out = buildTimelineEvents({
      ...BASE,
      incomes: [re({ id: 'i', type: 'income', dayOfMonth: 5 })],
  ...
  it('virement sortant → label « → nom », entrant → « ← nom » avec id+"-in"', () => {
    const out = buildTimelineEvents({
      ...BASE,
      outgoingTransfers: [
        re({ id: 'o', type: 'transfer', toAccountId: 'liv', label: 'Épargne', dayOfMonth: 8 }),
      ],
      incomingTransfers: [
  ...
  it('tri par cycle de paie (salaire 25 : 25,1,24)', () => {
    const days = buildTimelineEvents({
      ...
      salaryDay: 25,
      monthlyExpenses: [
        re({ id: 'd24', dayOfMonth: 24 }),
        re({ id: 'd1', dayOfMonth: 1 }),
        re({ id: 'd25', dayOfMonth: 25 }),
      ],
    }).map((x) => x.day);
  ...
  it('ignore les revenus sans dayOfMonth', () => {
    const out = buildTimelineEvents({
      ...BASE,
      incomes: [re({ id: 'i', type: 'income', dayOfMonth: null })],
    });
