...
@Component({
  ...
  template: `
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            (change)="onFile($event)" />
          @if (readError()) {
            <p class="mt-2 text-xs text-ib-red" role="alert">{{ 'budget.transactions.import.readError' | transloco }}</p>
          }
        </div>
      </div>
    }
    ...
    @if (step() === 2) {
      <div class="space-y-4">
        <p class="text-sm font-medium text-text-primary">{{ 'budget.transactions.import.step2' | transloco }}</p>
    ...
    @if (step() === 3) {
      <div class="space-y-4">
        <p class="text-sm font-medium text-text-primary">{{ 'budget.transactions.import.step3' | transloco }}</p>
          ...
          <table class="w-full text-sm">
            <thead class="bg-raised">
              <tr>
                ...
                <th class="px-3 py-2 text-left text-xs font-medium text-text-muted">{{ 'budget.transactions.import.date' | transloco }}</th>
                <th class="px-3 py-2 text-left text-xs font-medium text-text-muted">{{ 'budget.transactions.import.label' | transloco }}</th>
                <th class="px-3 py-2 text-right text-xs font-medium text-text-muted">{{ 'budget.transactions.import.amount' | transloco }}</th>
                <th class="px-3 py-2 text-left text-xs font-medium text-text-muted">{{ 'budget.transactions.import.category' | transloco }}</th>
            ...
            <tbody class="divide-y divide-border/40">
              @for (r of reviewRows(); track $index; let i = $index) {
                <tr [class.opacity-50]="!r.selected" class="hover:bg-hover/30 transition-colors">
                  ...
                  <td class="px-3 py-2">
                    @if (r.duplicate) {
                      <span class="inline-flex items-center rounded-full bg-ib-orange/10 px-2 py-0.5 text-xs font-medium text-ib-orange">
        ...
        <div class="flex justify-between items-center pt-2">
          <span class="text-sm text-text-muted">{{ toImport().length }} / {{ reviewRows().length }} {{ 'budget.transactions.import.selected' | transloco }}</span>
          <button
            type="button"
...
export class CsvImportWizard {
  ...
  protected setMapping(patch: Partial<CsvMapping>): void { this.mapping.update((m) => ({ ...m, ...patch })); }
  protected setSignedCol(col: number): void { this.mapping.update((m) => ({ ...m, amountMode: { kind: 'signed', col } })); }
