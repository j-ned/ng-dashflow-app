  ...
  template: `
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
      ...
      <section class="rounded-xl border border-border bg-surface overflow-hidden" #refCard>
        <div
          class="flex items-center justify-between px-4 py-3 bg-ib-red/5 border-b border-border/50"
        >
          <div class="flex items-center gap-2">
            ...
            <h3 class="text-[11px] font-semibold uppercase tracking-wider text-ib-red">
              {{ 'budget.bankAccount.expenses.monthlyTitle' | transloco }}
            </h3>
          </div>
          <button
            type="button"
        ...
        @if (monthlyExpenses().length > 0) {
          <div class="divide-y divide-border/20 px-3 py-1.5">
            @for (entry of monthlyExpenses(); track entry.id) {
              ...
              <div
                class="group flex items-center justify-between py-2 hover:bg-ib-red/3 rounded-lg px-1.5 -mx-1.5 transition-colors"
                [class.opacity-50]="passed"
              >
                <div class="flex items-center gap-2 min-w-0">
                  <div
                    class="flex h-7 w-7 items-center justify-center rounded-lg text-[10px] font-bold shrink-0"
                  ...
                  <div class="min-w-0">
                    ...
                    <div class="flex items-center gap-1 flex-wrap">
                      ...
                      @if (passed) {
                        <span class="text-[10px] text-ib-green font-medium">{{
                          'budget.bankAccount.expenses.debited' | transloco
                        }}</span>
                      } @else if (entry.dayOfMonth) {
                        <span class="text-[10px] text-text-muted">{{
                          'budget.bankAccount.expenses.onDay' | transloco: { day: entry.dayOfMonth }
                        }}</span>
                      }
                      @if (entry.endDate) {
                        <span
                          class="inline-flex items-center gap-0.5 rounded-md bg-ib-orange/10 px-1.5 py-0.5 text-[10px] font-medium text-ib-orange"
                        >
                          <app-icon name="calendar" size="9" />
                          {{
                            'budget.bankAccount.expenses.until'
                              | transloco: { date: (entry.endDate | date: 'MM/yyyy') }
                          }}
                        </span>
                      }
                    </div>
                ...
                <div class="flex items-center gap-1.5 shrink-0">
                  <span
                    ...
                    [class.text-ib-red]="!passed"
                    [class.text-text-muted]="passed"
                    >-{{ entry.amount | number: '1.2-2' }}&euro;</span
                  >
                  <app-entry-row-actions
                    [label]="entry.label"
                    variant="compact"
                    (edit)="edit.emit(entry)"
                    (delete)="delete.emit(entry.id)"
                  />
                </div>
              </div>
            }
          ...
          <div
            class="px-4 py-2.5 border-t border-border/50 bg-canvas/50 flex justify-between items-center"
          >
            <span class="text-[10px] font-medium text-text-muted uppercase tracking-wider">{{
              'budget.bankAccount.expenses.total' | transloco
            }}</span>
            <span class="text-sm font-mono font-bold text-ib-red"
          ...
          <div class="flex items-center justify-center py-8 px-4">
            <p class="text-xs text-text-muted text-center">
              {{ 'budget.bankAccount.expenses.monthlyEmpty' | transloco }}
            </p>
          </div>
        }
      </section>
      ...
      <section
        class="rounded-xl border border-border bg-surface overflow-hidden flex flex-col"
        [style.max-height.px]="refCardHeight()"
      >
        <div
          class="flex items-center justify-between px-4 py-3 bg-ib-orange/5 border-b border-border/50"
        >
          <div class="flex items-center gap-2">
            ...
            <h3 class="text-[11px] font-semibold uppercase tracking-wider text-ib-orange">
              {{ 'budget.bankAccount.expenses.annualTitle' | transloco }}
            </h3>
          </div>
          <button
            type="button"
        ...
        @if (annualExpenses().length > 0) {
          <div class="divide-y divide-border/20 px-3 py-1.5 overflow-y-auto flex-1">
            @for (entry of annualExpenses(); track entry.id) {
              <div
                class="group flex items-center justify-between py-2 hover:bg-ib-orange/3 rounded-lg px-1.5 -mx-1.5 transition-colors"
              >
                <div class="flex items-center gap-2 min-w-0">
                  <div
                    class="flex h-7 w-7 items-center justify-center rounded-lg bg-ib-orange/10 text-ib-orange text-[10px] font-bold shrink-0"
                    [title]="
                      entry.date ? '' : ('budget.bankAccount.expenses.annualNoDate' | transloco)
                    "
                  >
                    @if (entry.date) {
                      {{ entry.date | date: 'MMM' }}
                  ...
                  <div class="min-w-0">
                    <p class="text-[13px] font-medium text-text-primary truncate">
                      {{ entry.label }}
                    </p>
                    <div class="flex items-center gap-1 flex-wrap">
                      <span class="text-[10px] text-text-muted">{{
                        'budget.bankAccount.expenses.monthlyApprox'
                          | transloco: { value: (entry.amount / 12 | number: '1.2-2') }
                      }}</span>
                      @if (entry.category) {
                        <span class="text-[10px] text-text-muted">{{ entry.category }}</span>
                      }
                ...
                <div class="flex items-center gap-1.5 shrink-0">
                  <span class="text-[13px] font-mono font-bold text-ib-orange"
                    >-{{ entry.amount | number: '1.2-2' }}&euro;</span
                  >
                  <app-entry-row-actions
                    [label]="entry.label"
          ...
          >
            ...
            <div class="text-right">
              <span class="text-sm font-mono font-bold text-ib-orange"
                >{{ totalAnnualExpenses() | number: '1.2-2' }}
                {{ 'budget.bankAccount.expenses.annualSuffix' | transloco }}</span
              >
              <span class="text-[10px] text-text-muted ml-1">{{
                'budget.bankAccount.expenses.annualMonthlySuffix'
                  | transloco: { value: (monthlyAnnualExpenses() | number: '1.2-2') }
              }}</span>
            </div>
          </div>
        } @else {
          <div class="flex items-center justify-center py-8 px-4">
            <p class="text-xs text-text-muted text-center">
              {{ 'budget.bankAccount.expenses.annualEmpty' | transloco }}
            </p>
          </div>
        }
      </section>
      ...
      >
        <div
          class="flex items-center justify-between px-4 py-3 bg-ib-yellow/5 border-b border-border/50"
        >
          <div class="flex items-center gap-2">
            <app-icon name="banknote" size="14" class="text-ib-yellow" />
            <h3 class="text-[11px] font-semibold uppercase tracking-wider text-ib-yellow">
              {{ 'budget.bankAccount.expenses.spendingsTitle' | transloco }}
            </h3>
            <app-month-nav
        ...
        @if (monthSpendings().length > 0) {
          <div class="divide-y divide-border/20 px-3 py-1.5 overflow-y-auto flex-1">
            @for (entry of monthSpendings(); track entry.id) {
              <div
                class="group flex items-center justify-between py-2 hover:bg-ib-yellow/3 rounded-lg px-1.5 -mx-1.5 transition-colors"
              >
                <div class="flex items-center gap-2 min-w-0">
                  <div
                    class="flex h-7 w-7 items-center justify-center rounded-lg bg-ib-yellow/10 text-ib-yellow text-[10px] font-bold shrink-0"
                  >
                    @if (entry.date) {
                      {{ entry.date | date: 'dd' }}
                    } @else if (entry.dayOfMonth) {
                ...
                <div class="flex items-center gap-1.5 shrink-0">
                  <span class="text-[13px] font-mono font-bold text-ib-yellow"
                    >-{{ entry.amount | number: '1.2-2' }}&euro;</span
                  >
                  <app-entry-row-actions
                    [label]="entry.label"
          ...
          >
            ...
            <span class="text-sm font-mono font-bold text-ib-yellow"
              >{{ totalMonthSpendings() | number: '1.2-2' }} &euro;</span
            >
          </div>
        } @else {
          <div class="flex items-center justify-center py-8 px-4">
            <p class="text-xs text-text-muted text-center">
              {{
                'budget.bankAccount.expenses.spendingsEmpty'
                  | transloco: { month: spendingMonthLabel() }
              }}
            </p>
          </div>
        }
  ...
  constructor() {
    afterNextRender(() => {
      ...
      const ro = new ResizeObserver(([entry]) =>
        this.refCardHeight.set(entry.borderBoxSize[0].blockSize),
      );
      ro.observe(el);
    });
  }
