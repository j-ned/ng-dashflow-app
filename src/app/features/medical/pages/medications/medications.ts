...
@Component({
  ...
  imports: [ModalDialog, MedicationForm, RefillMedicationForm, MedicationStockBar, Icon, TranslocoPipe],
  host: { class: 'block space-y-6' },
  template: `
    <header class="flex items-center justify-between">
      <div>
        <h2 class="text-2xl font-bold text-text-primary">{{ 'medical.medication.title' | transloco }}</h2>
    ...
    <section [attr.aria-label]="'medical.medication.listLabel' | transloco" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      @for (med of medicationsWithStock(); track med.id) {
        <article class="group relative overflow-hidden rounded-xl border border-border bg-surface transition hover:border-ib-orange/30 hover:shadow-lg hover:shadow-ib-orange/5">
          ...
          <p class="text-xs text-text-muted mb-2">{{ 'medical.medication.patientLabel' | transloco }} : <span class="font-medium text-text-primary">{{ patientName(med.patientId) }}</span></p>
          ...
          <dl class="grid grid-cols-2 gap-2 text-xs mt-3">
            <div>
              <dt class="text-text-muted">{{ 'medical.medication.stockRemaining' | transloco }}</dt>
              <dd class="font-mono font-medium" [class.text-ib-green]="!med.isLow" [class.text-ib-red]="med.isLow">
                {{ 'medical.medication.stockUnits' | transloco: { remaining: med.remainingQuantity, total: med.quantity } }}
              </dd>
            </div>
            <div>
              <dt class="text-text-muted">{{ 'medical.medication.consumed' | transloco }}</dt>
              <dd class="font-mono text-ib-orange">{{ 'medical.medication.consumedUnits' | transloco: { value: med.consumedQuantity } }}</dd>
            </div>
            <div>
              <dt class="text-text-muted">{{ 'medical.medication.takeDaysRemaining' | transloco }}</dt>
              <dd class="font-mono text-ib-green">{{ 'medical.medication.daysShort' | transloco: { days: med.takeDaysRemaining } }}</dd>
            </div>
            <div>
              <dt class="text-text-muted">{{ 'medical.medication.restDaysRemaining' | transloco }}</dt>
              <dd class="font-mono text-text-muted">{{ 'medical.medication.daysShort' | transloco: { days: med.restDaysRemaining } }}</dd>
            </div>
            <div>
              <dt class="text-text-muted">{{ 'medical.medication.estimatedRunOut' | transloco }}</dt>
        ...
        <div class="col-span-full text-center py-16 rounded-xl border border-dashed border-border bg-surface">
          ...
          <p class="text-xs text-text-muted mt-1">{{ 'medical.medication.emptyHint' | transloco }}</p>
        </div>
      }
    </section>

    <app-modal-dialog #createModal [title]="'medical.medication.modalCreateTitle' | transloco" (closed)="onModalClosed()">
    ...
    <app-modal-dialog #editModal [title]="'medical.medication.modalEditTitle' | transloco" (closed)="onModalClosed()">
      @if (editModal.isOpen()) {
    ...
    <app-modal-dialog #refillModal [title]="'medical.medication.modalRefillTitle' | transloco" (closed)="onModalClosed()">
